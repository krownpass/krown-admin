...
export default function AdminCafeUpdatePage() {
    ...
    useEffect(() => {
        if (data?.cafe) {
            ...
            if (Array.isArray(wd)) {
                normalized = wd.map((d: string) => d.replace(/['"]+/g, "").trim());
                ...
                normalized = ["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"];
            }
    ...
    const updateCafe = useMutation({
        mutationFn: async (payload: UpdateCafeInput) => {
            return api.put(`/cafes/${cafeId}`, payload);
        },
        onSuccess: (res) => {
    ...
    // ---------- Render ----------
    if (isLoading) return <p className="text-center mt-10">Loading café data...</p>;
                                ...
                                {["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"].map((day) => {
                                    ...
                                    return (
                                        <button
                                            key={day}
                                            type="button"
                                            onClick={() => handleWorkingDaysChange(day as any)}
                      ...
                      ${selected
                        ...
                        <div className="flex gap-3">
                            <div className="w-1/2">
                                ...
                                <Input type="time" id="opening_time" {...form.register("opening_time")} />
                            </div>
                            <div className="w-1/2">
                                <Label htmlFor="closing_time">Closing Time</Label>
                                <Input type="time" id="closing_time" {...form.register("closing_time")} />
                            </div>
                        </div>
